...
const PressReleaseItem = ({ title, date, pdfLink }: { title: string; date: string; pdfLink: string }) => {
  return (
    <a href={pdfLink} target="_blank" rel="noopener noreferrer" className="block group">
      <div className="flex items-start justify-between p-5 bg-white rounded-lg border border-slate-200 hover:border-[#0099cc] hover:shadow-md transition-all duration-300">
        <div className="flex items-start">
          <FileText className="w-6 h-6 text-slate-400 mr-4 mt-1 flex-shrink-0" />
          <div>
            <h4 className="text-lg font-semibold text-slate-800 group-hover:text-[#0099cc] transition-colors">
              {title}
            </h4>
